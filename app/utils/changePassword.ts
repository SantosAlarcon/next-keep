...
export async function changePassword(lang: string, data: FormData, email: string, secret: string) {
	...
	if (password.length < 8) {
		...
		// Validation success
		// @ts-ignore
		const response = await appwriteAccount.updateRecovery(email, secret, password)
