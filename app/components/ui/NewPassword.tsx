...
const NewPassword = ({ lang }: { lang: string }) => {
	...
	const submitChangePassword = (data: FormData) => {
		setPending(true);
		// @ts-ignore
		changePassword(lang, data, userId, secret)
	...
	return (
		<>
			...
			<hr />
			<form id="newPasswordForm" action={(e) => submitChangePassword(e)} className={LoginStyles.login__page__form}>
				<FloatLabel>
					<Password
					...
					/>
					<label className={LoginStyles.login__page__form__label} htmlFor="password">{t("new-password-password")}</label>
				</FloatLabel>
				<FloatLabel>
					<Password
						className={LoginStyles.login__page__form__input}
						...
						toggleMask
					/>
					<label className={LoginStyles.login__page__form__label} htmlFor="confirm-password">{t("new-password-confirm-password")}</label>
				</FloatLabel>
				{/* @ts-ignore */}
				<Button type="submit" label={pending ? <BarLoader color="#eee" width="20px" height="20px" /> : t("new-password-title")} className="p-button-rounded" />
			</form>
		</>
	)
}
